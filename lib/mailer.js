...
function sendMail({ from, to, subject, html, user, pass }) {
  return new Promise((resolve, reject) => {
    console.log({ from, to, subject, html, user: !!user, pass: !!pass });

    let step = 0;
    let buffer = "";
    let settled = false;

    const EMAIL = user || process.env.mail;
    const PASSWORD = pass || process.env.pass;
    ...
    socket.on("data", (data) => {
      ...
      // Multi-line SMTP responses use "NNN-" for continuation and "NNN " for final line
      // Only process once we have a complete final line
    ...
    function handleResponse(msg) {
      ...
      if (step === 0 && msg.startsWith("220")) {
        ...
        // EHLO response is multi-line, wait for full response then AUTH
        step++;
